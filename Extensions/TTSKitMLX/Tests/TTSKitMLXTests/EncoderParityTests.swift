...
///
/// Goldens are exported by `scripts/export_voice_clone_goldens.py` into the
/// root package's `Tests/TTSKitTests/Resources/VoiceCloneGoldens/`; this nested
/// package cannot bundle the root package's resources, so they are referenced
/// via a `#filePath`-relative path. Two reference lengths are covered — 8 s
/// and its 4× concatenation (32 s) — so shape assertions double as proof that
/// the encoders are genuinely variable-length (the CoreML window is 10/15 s).
final class EncoderParityTests: XCTestCase {
    ...
    static var encoder: MlxVoiceCloneEncoder?
